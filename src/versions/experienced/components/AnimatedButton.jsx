...
const AnimatedButton = ({ text, href, download, onClick }) => {
  ...
  const handleMouseMove = (e) => {
    if (buttonRef.current) {
      ...
      xTo.current && xTo.current(x);
      yTo.current && yTo.current(y);
      ...
      if (divRef.current && textRef.current) {
        ...
        if (intersectionWidth > 0) {
          ...
          setGradient(
            `to right, black ${percentageCovered}%, white ${percentageCovered + 5}%`
          );
        } else {
          setGradient("to right, white 0%, white 100%");
        }
  ...
  return (
    <a
      ...
      className="button button--flex animated-button"
      ref={buttonRef}
